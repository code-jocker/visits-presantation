...
function DashboardSidebar({ isOpen, onClose, userRole = 'owner', user }: DashboardSidebarProps) {
  ...
  return (
    <>
      {isOpen && (
        <div className="fixed inset-0 bg-black/40 bg-opacity-50 z-40 lg:hidden" onClick={onClose} />
      )}

      <aside
        className={`w-64 lg:w-72 bg-[#1A3263] text-white h-full flex flex-col transition-transform duration-300 ease-in-out lg:translate-x-0 fixed lg:relative z-50 lg:z-auto max-w-[80vw] ${
      ...
      >
        ...
        <nav className="flex-1 px-4 pt-10 flex flex-col min-h-0">
          ...
          <div
            className="flex-1 overflow-y-scroll min-h-0 hide-scrollbar"
                    ...
                    {hasChildren ? (
                      <>
                        ...
                        >
                          <Icon
                            size={18}
                            className={location.pathname === item.path ? 'text-[#1A3263]' : 'text-gray-300'}
                          />
                          <span
                            className={`font-bold flex-1 text-left ${
                              location.pathname === item.path ? 'text-[#1A3263]' : 'text-gray-300'
                            }`}
                          >
                            {item.label}
                          </span>
                            ...
                            <ul className="space-y-2">
                              {item.children?.map((child) => (
                                <li key={child.path} className="relative">
                                  <div className="absolute top-4 w-3 h-px bg-white/30" />
                                  <Link
                      ...
                      >
                        <Icon
                          size={18}
                          className={isActive(item.path, item.exact) ? 'text-[#1A3263]' : 'text-gray-300'}
                        />
                        <span
                          className={`font-bold ${
                            isActive(item.path, item.exact) ? 'text-[#1A3263]' : 'text-gray-300'
                          }`}
                        >
                          {item.label}
                        </span>
          ...
          {user && checkPermissions(user, 'setting:read') && (
            <div className="flex-shrink border-t border-white/20">
              ...
              >
                <settingsItem.icon
                  size={18}
                  className={isActive(settingsItem.path) ? 'text-[#1A3263]' : 'text-gray-300'}
                />
                <span
                  className={`font-bold ${
                    isActive(settingsItem.path) ? 'text-[#1A3263]' : 'text-gray-300'
                  }`}
                >
                  {settingsItem.label}
                </span>
