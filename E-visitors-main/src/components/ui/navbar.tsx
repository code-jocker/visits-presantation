...
function Navbar({ hideLinks = false }: NavbarProps) {
  ...
  return (
    <div className='flex flex-row justify-between items-center px-4 sm:px-8 py-4 sm:py-5 rounded-t-2xl sm:rounded-t-[40px] gap-2 bg-[#1A3263]'>
      {/* Logo */}
      <div className='flex items-center gap-2 sm:gap-4'>
        <img src={evsLogo} alt='EVS Logo' className='w-8 h-8 sm:w-14 sm:h-14 object-contain bg-white/10 rounded-lg sm:rounded-xl p-1' />
        <div className='flex flex-col'>
          <span className='text-xs sm:text-2xl font-black text-white tracking-tight'>
            E-visitor platform
          </span>
          <span className='hidden sm:block text-[10px] font-bold text-white/50 uppercase tracking-[0.2em]'>
            Digital Registration Station
          </span>
        </div>
      </div>
      ...
      {!hideLinks && (
        <nav className='hidden md:block'>
          <ul className='flex gap-6 text-white font-bold'>
            <Link to='/' className='hover:text-gray-300 cursor-pointer text-xl flex items-center gap-2'>
              <FaHome size={16} />
              Home
            </Link>
            <Link to='/about' className='hover:text-gray-300 cursor-pointer text-xl flex items-center gap-2'>
              <FaInfoCircle size={16} />
              About
            </Link>
            <Link to='/services' className='hover:text-gray-300 cursor-pointer text-xl flex items-center gap-2'>
              <FaCogs size={16} />
              Services
            </Link>
            <Link to='/contact' className='hover:text-gray-300 cursor-pointer text-xl flex items-center gap-2'>
              <FaEnvelope size={16} />
              Contact
            </Link>
          </ul>
