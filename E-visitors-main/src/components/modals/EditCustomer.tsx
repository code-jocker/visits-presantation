...
interface EditCustomerProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (customerData: EditCustomerFormData) => void;
  customerData?: Partial<EditCustomerFormData>;
}

type EditCustomerFormData = {
  companyName: string
  // Company fields used by this form UI
  companyEmail: string
  companyPhone: string
  industry: string
  companySize: string
  ...
  // Legacy/backward-compat fields (may be used by other parts of the app)
  email?: string
  phone?: string
  plan?: string
...
function EditCustomer({ isOpen, onClose, onSubmit, customerData }: EditCustomerProps) {
  ...
  const [sectors, setSectors] = useState<Division[]>([]);
  const [cells, setCells] = useState<Division[]>([]);
  ...
  const [formData, setFormData] = useState<EditCustomerFormData>({
    ...
    // legacy fields (kept to match older incoming data)
    email: "",
    phone: "",
    plan: "",
  ...
  useEffect(() => {
    if (isOpen && customerData) {
      setFormData({
        ...
        companyEmail: (customerData as any).companyEmail || (customerData as any).email || "",
        companyPhone: (customerData as any).companyPhone || (customerData as any).phone || "",
        industry: (customerData as any).industry || "",
        companySize: (customerData as any).companySize || "",
        website: (customerData as any).website || "",
        subscriptionPlan: (customerData as any).subscriptionPlan || (customerData as any).plan || "",

        // legacy fields (may exist on older payloads)
        email: (customerData as any).email || "",
        phone: (customerData as any).phone || "",
        plan: (customerData as any).plan || "",

        startDate: (customerData as any).startDate || "",
        endDate: (customerData as any).endDate || "",
        country: (customerData as any).country || "",
        province: (customerData as any).province || "",
        district: (customerData as any).district || "",
        sector: (customerData as any).sector || "",
        cell: (customerData as any).cell || "",
        village: (customerData as any).village || "",
        ...
        position: (customerData as any).position || "",
        department: (customerData as any).department || ""
      });
    }
