...
export default function EditUserModal({ isOpen, onClose, onSubmit, user }: EditUserModalProps) {
  ...
  const validateForm = () => {
    ...
    // Guard against undefined/null values to avoid runtime crashes.
    const safe = (v: unknown) => (v ?? '').toString().trim()

    if (!safe(formData.name)) newErrors.name = 'Name is required'
    if (!safe(formData.email)) newErrors.email = 'Email is required'
    if (!safe(formData.phone)) newErrors.phone = 'Phone is required'
    if (!safe(formData.company)) newErrors.company = 'Company is required'
