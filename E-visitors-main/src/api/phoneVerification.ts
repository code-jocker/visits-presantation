...
export async function validatePhoneWithNumverify(params: {
  ...
  if (!NUMVERIFY_API_KEY || numverifyDisabled) {
    throw new Error('Numverify phone validation is unavailable')
  }
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    try {
      ...
      if (!res.ok) {
        if (res.status === 401) {
          numverifyDisabled = true
          throw new Error('Numverify API key is not authorized')
        }
