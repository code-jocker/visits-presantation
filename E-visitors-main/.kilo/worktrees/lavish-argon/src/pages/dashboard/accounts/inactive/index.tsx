...
function InactiveCustomers() {
  ...
  useEffect(() => {
    usersApi.getAll({ take: 1000 })
      .then(res => setCustomers((res.result || []).filter((u: any) => u.status?.toLowerCase() === 'inactive')))
      .catch(() => setError('Failed to load inactive customers'))
      .finally(() => setLoading(false))
  }, [])

  const filtered = customers.filter(c =>
    [c.fullName, c.email, c.company].filter(Boolean).join(' ').toLowerCase().includes(searchTerm.toLowerCase())
  )
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {[
          { label: 'Inactive Customers', value: customers.length, icon: FaBuilding, color: 'bg-gray-500' },
          { label: 'Lost Revenue', value: '—', icon: FaDollarSign, color: 'bg-gray-500' },
          { label: 'Inactive Users', value: customers.length, icon: FaUsers, color: 'bg-gray-500' },
          { label: 'Reactivation Rate', value: '—', icon: FaCalendarAlt, color: 'bg-orange-500' },
        ].map((s, i) => (
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filtered.length === 0 ? (
                  <tr><td colSpan={6} className="px-6 py-8 text-center text-gray-500">No inactive customers found.</td></tr>
                ) : filtered.map(c => (
                  <tr key={c.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <div className="flex space-x-2">
                        ...
                        <button onClick={() => { setSelectedCustomer(c); setShowEditModal(true) }} className="text-green-600 hover:text-green-900"><FaEdit size={18} /></button>
                        <button onClick={() => window.alert('Reactivation coming soon.')} className="text-orange-600 hover:text-orange-900" title="Reactivate"><FaPlay size={18} /></button>
                      </div>
                    </td>
                  </tr>
                ))}
