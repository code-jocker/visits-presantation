...
function ScanningPage() {
  const [selectedVisitorPhoto, setSelectedVisitorPhoto] = useState<string>('')
  const [selectedVisitorId, setSelectedVisitorId] = useState<string | number | null>(null)

  const fetchAndSetVisitorPhoto = async (visitorId: string | number) => {
    try {
      const res = await visitorApi.getById(visitorId)
      const photo = res?.result?.profilePhoto
      setSelectedVisitorPhoto(photo || '')
    } catch {
      setSelectedVisitorPhoto('')
    }
  }
