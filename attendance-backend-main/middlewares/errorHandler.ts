...
const returnErrorToUser: ErrorRequestHandler = (errors, _req, res, next) => {
  ...
  error.statusCode = Number.isInteger(statusCode) && statusCode > 0 ? statusCode : 500;
  error.message = error.message || "Internal Server Error";

  if (error.type === "entity.too.large" || error.status === 413) {
    error.statusCode = 413;
    error.message = "Request body is too large. Reduce the image size or increase JSON_BODY_LIMIT.";
  }
  ...
  if (process.env.NODE_ENV === "production" || process.env.NODE_ENV === "test") {
    console.log("Getting this error", {
      ...
      // Helpful for tsoa validation errors
    ...
    if (error.name === "SequelizeValidationError") {
      ...
      if (validationErrors.length > 0) {
        ...
        if (field === 'mobile' && validatorKey === 'is') {
          message = "Phone number must contain 10-15 digits. Please enter the full number with country code.";
        } else if (field === 'email' && validatorKey === 'isEmail') {
          message = "Please enter a valid email address (e.g., example@email.com).";
        } else if (field === 'phone' && validatorKey === 'isNumeric') {
